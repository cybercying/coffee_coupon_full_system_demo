...
import 'package:logging_appenders/src/base_appender.dart' show BaseLogAppender;
// ignore: implementation_imports
...
class RotatingFileAppender2 extends BaseLogAppender {
  bool immediatelyFlush;
  RotatingFileAppender2({
  ...
  RandomAccessFile _getRandomAccessFile() =>
      _randomAccessFile ??= _outputFile.openSync(mode: FileMode.writeOnlyAppend);
  ...
  void handle(LogRecord record) {
    if (immediatelyFlush) {
      _getRandomAccessFile()
        ..writeStringSync(formatter.format(record))
        ..writeStringSync('\n')
        ..closeSync();
